### kafka的相关机制，使用过程中的问题，是如何解决的：

> ​	一次消费的动作太慢，花费时间过长，导致消费者下线(消费者消费消息的同时也在发送心跳给broker)
>
> ​	如果poll 消息的间隔超过了rebalanceTimeoutMs，则consumer告知broker主动离开消费组，触发rebalance
>
> ​	解决办法：
>
> ​	1.启用线程池，异步执行
...
​	可能你的消费端出了问题，不消费了；或者消费的速度极其慢。接着就坑爹了，可能你的消息队列集群的磁盘都快写满了，都没人消费，这个时候怎么办？或者是这整个就积压了几个小时，你这个时候怎么办？或者是你积压的时间太长了，导致比如 RabbitMQ 设置了消息过期时间后就没了怎么办？
